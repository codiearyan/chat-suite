...
export default function StarField() {
  ...
  const createNebulae = () => {
    ...
    const nebulaCount = 3 // Reduced count for better performance
    
    const createNebulaLayer = (size: number, complexity: number, opacity: number) => {
    ...
    for (let i = 0; i < nebulaCount; i++) {
      ...
      layers.forEach(layer => {
        const nebulaLayer = createNebulaLayer(layer.size, layer.complexity, layer.opacity)
        nebulaGroup.add(nebulaLayer)
      })
      
      // Add volumetric effect using multiple rotated planes
      const planeCount = 5
  ...
  useFrame(({ clock }) => {
    ...
    if (nebulae.current) {
      ...
      nebulae.current.children.forEach((nebulaGroup, i) => {
        ...
        // Animate internal layers
        nebulaGroup.children.forEach((layer, j) => {
          if (layer instanceof THREE.Points) {
            layer.rotation.y = Math.sin(clock.getElapsedTime() * 0.1 + j) * 0.1
            layer.material.opacity = 0.05 + Math.sin(clock.getElapsedTime() * 0.3 + j) * 0.05
          }
          if (layer instanceof THREE.Mesh) {
            layer.rotation.z += 0.001
            layer.material.opacity = 0.02 + Math.sin(clock.getElapsedTime() * 0.2 + j) * 0.01
          }
        })
      })
    }
